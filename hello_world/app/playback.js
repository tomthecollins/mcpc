define(function (require) {
  ...
  for (i = 0; i < instr_keys.length; i++){
    if (c.Instruments[instr_keys[i]].samples !== undefined){
      ...
      var curr_instr_str = 'var ' + instr_keys[i] + ' = new Tone.PolySynth(c.Instruments.' +
      instr_keys[i] + '.polyphony, Tone.SimpleSynth, ' +
      'c.Instruments.'+ instr_keys[i] + '.volEnvEtc).toMaster();';
      eval(curr_instr_str);
    }
    ...
    for (inam = 0; inam < unq_instr_nam.length; inam++){
      if (samples_lookup[unq_instr_nam[inam]] !== undefined){
        ...
        switch (unq_instr_nam[inam]) {
          case 'piano_1':
            var tmp = require('./samples_idx/keyboards/pianos/piano_1');
            break;
          case 'piano_2':
            var tmp = require('./samples_idx/keyboards/pianos/piano_2');
            break;
          case 'synth_bass_1':
            var tmp = require('./samples_idx/plucked_strings/bass_guitars/synth_bass_1');
            break;
          case 'electric_guitar_1':
            var tmp = require('./samples_idx/plucked_strings/electric_guitars/electric_guitar_1');
            break;
          case 'drum_kit_1':
            var tmp = require('./samples_idx/unpitched_percussion/drum_kits/drum_kit_1');
            break;
          default:
            var tmp = require('./samples_idx/keyboards/pianos/synth_piano_1');
            break;
        }
          ...
          for (i = 0; i < relMapKeys.length; i++){
            ...
            if (typeof relMap[j][prop_str] == "object" &&
              ...
              if (mapLookup[prop_str] >= relMap[j][prop_str][0] &&
                  // There is a <= sign here to handle instances of velocity =
                  // 1, which otherwise don't get assigned to anything.
  ...
  function tonejs_playback(
    ...
    if (left_most_ontime == undefined){
      left_most_ontime = 0;
    }
    if (samples_idx == undefined) {
      samples_idx = "../lib/samples_idx/";
    }
    if (samples_lookup == undefined) {
      samples_lookup = {
        "piano_1": "keyboards/pianos/piano_1",
    ...
    var rel_notes = [];
    for (i = 0; i < comp.notes.length; i++){
      if (comp.notes[i].acousticDescriptor !== undefined &&
          comp.notes[i].acousticDescriptor.onAdj !== undefined){
        var ontime_to_test =
        comp.notes[i].ontime + comp.notes[i].acousticDescriptor.onAdj;
      }
      else {
        var ontime_to_test = comp.notes[i].ontime;
      }
      if (ontime_to_test >= left_most_ontime){
        rel_notes.push(comp.notes[i]);
      }
    }
    ...
    var offtimes = [];
    for (i = 0; i < comp.notes.length; i++){
      offtimes.push(comp.notes[i].offtime);
    }
